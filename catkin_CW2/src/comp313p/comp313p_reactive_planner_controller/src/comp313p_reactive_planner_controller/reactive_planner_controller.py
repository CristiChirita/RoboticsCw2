...
class ReactivePlannerController(PlannerControllerBase):
    ...
    def checkIfPathCurrentPathIsStillGood(self):
        ...
        # This method needs to check if the current path, whose
	...
	print "Number of waypoints: " + str(self.currentPlannedPath.numberOfWaypoints)
	if self.currentPlannedPath.numberOfWaypoints == 0:
		self.controller.stopDrivingToCurrentGoal()
		return
	for cell in self.currentPlannedPath.waypoints:
		# print "Testing Cell: " + str(cell.coords)
		# print "Label: " + str(cell.label)
		# print "X: " + str(cell.coords[0]) + " Y: " + str(cell.coords[1])
		if self.occupancyGrid.grid[cell.coords[0]][cell.coords[1]] == 1:
			print "Obstacle Found in Path at " + str(cell.coords)
			try:
				while self.currentPlannedPath.waypoints.pop():
					continue
			except IndexError:
				pass
#			self.currentPlannedPath.goalReached = False
#			self.currentPlannedPath.numberOfWaypoints = 0
#			self.currentPlannedPath.travelCost = 0
			self.controller.stopDrivingToCurrentGoal()
			return
	self.currentPlannedPath.waypoints.popleft()
        return
    ...
    def driveToGoal(self, goal):
        ...
        while (goalReached is False) & (rospy.is_shutdown() is False):
            ...
            print 'Planning a new path: start=' + str(startCellCoords) + '; goal=' + str(goalCellCoords)
